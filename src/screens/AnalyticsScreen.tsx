...
import { X, TrendingUp, ArrowRight, Lightbulb, Target, Users, ChevronDown, Crown } from 'lucide-react-native';
...
export default function AnalyticsScreen() {
    ...
    useEffect(() => {
        setInsights(generateInsights(habits, logs, 0));
    }, [habits, logs]);
            ...
            <View style={{ gap: 20 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 12 }}>
                    ...
                    <View style={[styles.card, { flex: 1, padding: 16 }]}>
                        ...
                        {habits.length > 0 ? (() => {
                            const { risk_percentage, risk_level, primary_factor } = calculateStreakRisk(habits[0].id, logs, 0);
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
                ...
                {activeTab === 'advance' && (
                    isPremium ? renderAdvance() : (
                        <View style={{ alignItems: 'center', justifyContent: 'center', padding: 40, marginTop: 40, backgroundColor: 'white', borderRadius: 16 }}>
                            <Crown size={64} color="#F59E0B" style={{ marginBottom: 20 }} />
                            <Text style={{ fontSize: 24, fontWeight: 'bold', color: theme.colors.text, marginBottom: 12, textAlign: 'center' }}>Premium Feature</Text>
                            <Text style={{ fontSize: 16, color: theme.colors.textSecondary, textAlign: 'center', marginBottom: 24, lineHeight: 24 }}>Upgrade to HabitRat Pro to unlock deep insights, burnout predictions, and optimal execution windows.</Text>
                            <TouchableOpacity
                                style={{ backgroundColor: '#8B5CF6', paddingHorizontal: 32, paddingVertical: 14, borderRadius: 24 }}
